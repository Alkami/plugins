...
Future<void> main() async {
  ...
  testWidgets('loadUrl with headers', (WidgetTester tester) async {
    ...
    final String content = await controller.runJavascriptReturningResult('document.documentElement.innerText');
  ...
  group('SurfaceAndroidWebView', () {
    ...
    testWidgets('inputs are scrolled into view when focused', (WidgetTester tester) async {
      ...
      final WebViewController controller = await controllerCompleter.future;
      await pageLoaded.future;
      final String viewportRectJSON =
          await _runJavaScriptReturningResult(controller, 'JSON.stringify(viewport.getBoundingClientRect())');
      final Map<String, dynamic> viewportRectRelativeToViewport = jsonDecode(viewportRectJSON) as Map<String, dynamic>;

      // Check that the input is originally outside of the viewport.

      final String initialInputClientRectJSON =
          await _runJavaScriptReturningResult(controller, 'JSON.stringify(inputEl.getBoundingClientRect())');
      ...
      expect(initialInputClientRectRelativeToViewport['bottom'] <= viewportRectRelativeToViewport['bottom'], isFalse);
      ...
      expect(lastInputClientRectRelativeToViewport['top'] >= viewportRectRelativeToViewport['top'], isTrue);
      expect(lastInputClientRectRelativeToViewport['bottom'] <= viewportRectRelativeToViewport['bottom'], isTrue);

      expect(lastInputClientRectRelativeToViewport['left'] >= viewportRectRelativeToViewport['left'], isTrue);
      expect(lastInputClientRectRelativeToViewport['right'] <= viewportRectRelativeToViewport['right'], isTrue);
    }, skip: _skipDueToIssue86757);
  });
  ...
  group('NavigationDelegate', () {
    ...
    testWidgets('onWebResourceError', (WidgetTester tester) async {
      final Completer<WebResourceError> errorCompleter = Completer<WebResourceError>();
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(controller.runJavascriptReturningResult('iframeLoaded'), completion('true'));
      expect(
        controller
            .runJavascriptReturningResult('document.querySelector("p") && document.querySelector("p").textContent'),
        completion('null'),
      );
    },
  );
